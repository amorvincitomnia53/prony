...
public:
    ...
    OutputData(SizeType count, T* addr)
        : _type(GetDataType<T>()), _size(unsigned(count), addr), _addr(addr)
    {
    }
    ...
    OutputData(T (&addr)[N])
        : _type(GetDataType<T>()), _size(addr), _addr(addr)
    {
    }
    ...
    OutputData(std::array<T, N>& a)
        : _type(GetDataType<T>()), _size(a), _addr(a.data())
    {
    }
    ...
    OutputData(std::vector<T>& v)
        : _type(GetDataType<T>()), _size(v), _addr(v.data())
    {
    }
