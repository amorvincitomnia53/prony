...
public:
    ...
    ProgVarLoc(ProgramName program)
        OGLPLUS_NOEXCEPT(true)
        : _program(GetGLName(program)), _location(-1)
    {
    }
    ...
    ProgVarLoc(LocOpsParam param, ProgramName program, StrCRef identifier)
        : ProgVarLocOps<VarTag>(param), _program(GetGLName(program)), _location(ProgVarLocOps<VarTag>::GetLocation(
                                                                          program,
                                                                          identifier,
                                                                          true))
    {
    ...
    ProgVarLoc(
        ...
        ProgramName program,
        StrCRef identifier,
        bool active_only) : ProgVarLocOps<VarTag>(param), _program(GetGLName(program)), _location(ProgVarLocOps<VarTag>::GetLocation(program,
                                                                                            identifier,
                                                                                            active_only))
    {
    }
