...
class ProgVarBaseSetOps<OpsTag, VarTag, tag::NativeTypes, T, M>
    : public ProgVarSetters<OpsTag, VarTag, tag::NativeTypes>, public ProgVarCallers<OpsTag, T>
{
private:
        ...
        const V* v)
    {
        ...
        OGLPLUS_CHECK_CTXT(
            ProgVarError,
            Program(ProgramName(program)).Index(base_location));
        _do_set_v<N - 4, LI, V>(
            _set_mode<N - 4>(),
            program,
            base_location,
...
protected:
    ...
    static void _do_set(GLuint program, LI location, V... v)
    {
        ...
        OGLPLUS_CHECK_CTXT(
            ProgVarError,
            Program(ProgramName(program)).Index(location));
    }
#else
    template <typename LI, typename V>
...
class ProgVarBaseSetOps<OpsTag, VarTag, tag::MatrixTypes, T, M>
    : public ProgVarSetters<OpsTag, VarTag, tag::MatrixTypes>, public ProgVarCallers<OpsTag, T>
{
private:
