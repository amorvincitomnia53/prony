...
class ObjCommonOps<tag::Program>
    : public ProgramName, public ObjBindingOps<tag::Program>
{
protected:
...
public:
    ...
    ObjCommonOps(ObjCommonOps&& temp)
        OGLPLUS_NOEXCEPT(true)
        : _base1(static_cast<_base1&&>(temp)), _base2(static_cast<_base2&&>(temp))
    {
    }

    ObjCommonOps(const ObjCommonOps& that)
        OGLPLUS_NOEXCEPT(true)
        : _base1(static_cast<const _base1&>(that)), _base2(static_cast<const _base2&>(that))
    {
    }
...
public:
    ...
    GLint GetIntParam(ProgramParameter query) const
    {
        ...
        (_obj_name(), GLenum(query), &result);
        OGLPLUS_VERIFY(
            GetProgramiv,
            ObjectError,
            Object(*this).EnumParam(query));
        return result;
    }
    ...
    void TransformFeedbackVaryings(
        ...
        TransformFeedbackMode mode) const;
    ...
    struct IteratedShaderName
        ...
        IteratedShaderName(
            ...
            unsigned index) : ShaderName(context._shader_names.at(index))
        {
        }
    };
    ...
    GLint* ComputeWorkGroupSize(GLint (&result)[3]) const
    {
        ...
        OGLPLUS_VERIFY(
            GetProgramiv,
            ObjectError,
            Object(*this).EnumParam(ProgramParameter(GL_COMPUTE_WORK_GROUP_SIZE)));
        return result;
    }
#endif
        ...
        StrCRef identifier)
    {
        ...
        OGLPLUS_CHECK(
            BindAttribLocation,
            ProgVarError,
            Program(*this).Identifier(identifier).Index(GLuint(vertex_attrib_slot)));
    }
};
...
struct ProgXFBModeAndNames {
    ...
    ProgXFBModeAndNames(ProgXFBModeAndNames&& pman, const GLchar* name)
        : prog(pman.prog), mode(pman.mode), names(std::move(pman.names))
    {
        names.push_back(name);
    }

    ProgXFBModeAndNames(ProgXFBModeAndNames&& tmp)
        : prog(tmp.prog), mode(tmp.mode), names(std::move(tmp.names))
    {
    }
...
public:
    ...
    ShaderProgram(
        ShaderType shader_type,
        GLSLString&& source) : Program(_make(shader_type, source))
    {
        _check();
    }
    ...
    ShaderProgram(
        ShaderType shader_type,
        GLSLString&& source,
        ObjectDesc&& object_desc) : Program(_make(shader_type, source), std::move(object_desc))
    {
        _check();
    }
    ...
    ShaderProgram(
        ShaderType shader_type,
        GLSLStrings&& source) : Program(_make(shader_type, source))
    {
        _check();
    }
    ...
    ShaderProgram(
        ShaderType shader_type,
        const GLSLSource& glsl_source) : Program(_make(shader_type, glsl_source.Count(), glsl_source.Parts()))
    {
        _check();
    }
    ...
    ShaderProgram(
        ...
        ObjectDesc&& object_desc) : Program(_make(shader_type, glsl_source.Count(), glsl_source.Parts()),
...
public:
    ...
    QuickProgram(
        ...
        const Sequence<ShaderName>& shaders) : Program(std::move(object_desc))
    {
        AttachShaders(shaders);
        Link();
        Use();
