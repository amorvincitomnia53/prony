...
public:
    ...
    StrCRefTpl(const std::basic_string<Char>& sstr)
        OGLPLUS_NOEXCEPT(true)
        : _c_str(sstr.c_str()), _size(sstr.size())
    {
        _validate();
    }
    ...
    StrCRefTpl(const std::vector<Char>& cvec)
        OGLPLUS_NOEXCEPT(true)
        : _c_str(cvec.data()), _size(cvec.size())
    {
        _validate();
    }
    ...
    {
        return (a.size() == b.size()) && (std::strncmp(a.begin(), b.begin(), a.size()) == 0);
    }
    ...
    static bool Equal(const StrCRefTpl& a, const Char* b)
    {
        ...
        return (a.size() == size) && (std::strncmp(a.begin(), b, a.size()) == 0);
    }
...
template <typename Char>
inline StrCRefChainTpl<Char, StrCRefTpl<Char>, StrCRefTpl<Char>>
...
template <typename Char, typename Left, typename Right>
inline StrCRefChainTpl<Char, StrCRefChainTpl<Char, Left, Right>, StrCRefTpl<Char>>
