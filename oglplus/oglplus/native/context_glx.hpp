...
protected:
    ...
    ContextGLX(Current_)
        : _display(::glXGetCurrentDisplay()), _context(::glXGetCurrentContext())
    {
        if (!_display)
            HandleNoGLXDisplay();
        if (!_context)
...
public:
    friend bool operator==(const ContextGLX& a, const ContextGLX& b)
        OGLPLUS_NOEXCEPT(true)
    {
    ...
    friend bool operator<(const ContextGLX& a, const ContextGLX& b)
        OGLPLUS_NOEXCEPT(true)
    {
        return (a._display < b._display) || ((a._display == b._display) && (a._context < b._context));
    }
