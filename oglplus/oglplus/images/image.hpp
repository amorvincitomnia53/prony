...
protected:
    ...
    Image(void)
        OGLPLUS_NOEXCEPT(true)
        : _width(0), _height(0), _depth(0), _channels(0), _convert(nullptr)
    {
    }

public:
    Image(const Image& that)
        : _width(that._width), _height(that._height), _depth(that._depth), _channels(that._channels), _type(that._type), _storage(that._storage), _convert(that._convert), _format(that._format), _internal(that._internal)
    {
    }

    Image(Image&& tmp)
        OGLPLUS_NOEXCEPT(true)
        : _width(tmp._width), _height(tmp._height), _depth(tmp._depth), _channels(tmp._channels), _type(tmp._type), _storage(std::move(tmp._storage)), _convert(tmp._convert), _format(tmp._format), _internal(tmp._internal)
    {
    }
    ...
    Image(
        ...
        SizeType depth,
        SizeType channels,
        const T* data) : _width(width), _height(height), _depth(depth), _channels(channels), _type(PixelDataType(GetDataType<T>())), _storage(oglplus::aux::AlignedPODArray(data,
                                                                                                                                         std::size_t(_width * _height * _depth * _channels))),
                         _convert(&_do_convert<T>), _format(_get_def_pdf(unsigned(channels))), _internal(_get_def_pdif(unsigned(channels)))
    {
    }
    ...
    Image(
        ...
        PixelDataInternalFormat internal) : _width(width), _height(height), _depth(depth), _channels(channels), _type(PixelDataType(GetDataType<T>())), _storage(oglplus::aux::AlignedPODArray(data,
                                                                                                                                                            std::size_t(_width * _height * _depth * _channels))),
                                            _convert(&_do_convert<T>), _format(format), _internal(internal)
    {
    }
