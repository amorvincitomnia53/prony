...
public:
    ...
    public:
        ...
        void operator()(
            int& x, int& y, int& z,
            double w, double h, double d) const
        {
    ...
    public:
        SamplerTpl(
            ...
            const SampleFunc& sample = SampleFunc()) : _transf(transf), _sample(sample), _image(nullptr), _ori_x(0), _ori_y(0), _ori_z(0)
        {
        }
    ...
    struct MatrixTransformSampler
        ...
        MatrixTransformSampler(
            ...
            const SampleFunc& sample = SampleFunc()) : SamplerTpl<MatrixCoordTransform, SampleFunc>(MatrixCoordTransform(transf),
                                                           sample)
        {
        }
    };
    ...
    FilteredImage(
        ...
        Filter filter,
        Sampler sampler,
        Extractor extractor) : Image(input.Width(),
                                   input.Height(),
                                   input.Depth(),
                                   CH,
                                   &TypeTag<T>())
