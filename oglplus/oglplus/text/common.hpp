...
public:
    ...
    DefaultRendererTpl(
        ...
        const Sequence<ShaderName>& shaders) : ConcreteRenderer(parent, shaders), _projection_matrix(Base::_get_program(), "oglpProjectionMatrix"), _camera_matrix(Base::_get_program(), "oglpCameraMatrix"), _layout_matrix(Base::_get_program(), "oglpLayoutMatrix"), _align_coef(Base::_get_program(), "oglpAlignCoef", false), _dir_coef(Base::_get_program(), "oglpDirCoef", false)
    {
    ...
    DefaultRendererTpl(DefaultRendererTpl&& tmp)
        : Base(static_cast<Base&&>(tmp)), _projection_matrix(std::move(tmp._projection_matrix)), _camera_matrix(std::move(tmp._camera_matrix)), _layout_matrix(std::move(tmp._layout_matrix)), _align_coef(std::move(tmp._align_coef)), _dir_coef(std::move(tmp._dir_coef))
    {
    }
#endif
