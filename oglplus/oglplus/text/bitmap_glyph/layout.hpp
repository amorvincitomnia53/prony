...
private:
    ...
    {
        return (_data._offset >= 0) && (_data._capacity > 0) && (_data._storage);
    }
...
public:
    BitmapGlyphLayoutTpl(
        ...
        SizeType max_len) : _parent(parent), _font(font), _data(max_len)
    {
    ...
    BitmapGlyphLayoutTpl(const BitmapGlyphLayoutTpl& other)
        : _parent(other._parent), _font(other._font), _data(other._data._capacity), _pages(other._pages)
    {
        BitmapGlyphAllocateLayoutData(_parent, _data);
        assert(_is_ok());
    }
    ...
    BitmapGlyphLayoutTpl(BitmapGlyphLayoutTpl&& tmp)
        : _parent(tmp._parent), _font(std::move(tmp._font)), _data(std::move(tmp._data)), _pages(std::move(tmp._pages))
    {
