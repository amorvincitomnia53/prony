...
public:
    ...
    PangoCairoLayout(PangoCairoLayout&& tmp)
        : _parent(tmp._parent), _font(std::move(tmp._font)), _capacity(tmp._capacity), _curr_width(tmp._curr_width), _width(tmp._width), _height(tmp._height), _surface(std::move(tmp._surface)), _data(std::move(tmp._data))
    {
    }
