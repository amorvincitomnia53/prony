...
public:
    ...
    ObjZeroOps(ObjZeroOps&& temp)
        OGLPLUS_NOEXCEPT(true)
        : _base(static_cast<_base&&>(temp)), target(temp.target)
    {
    }

    ObjZeroOps(const ObjZeroOps& that)
        OGLPLUS_NOEXCEPT(true)
        : _base(static_cast<const _base&>(that)), target(that.target)
    {
    }
        ...
        const void* data)
    {
        ...
        OGLPLUS_CHECK(
            TextureImage2DEXT,
            ObjectError,
            Object(*this).BindTarget(tex_target).EnumParam(internal_format).Index(level));
        return *this;
    }
        ...
        const void* data)
    {
        ...
        OGLPLUS_CHECK(
            TextureImage2DEXT,
            ObjectError,
            Object(*this).BindTarget(TextureTarget(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face)).EnumParam(internal_format).Index(level));
        return *this;
    }
