...
public:
    ...
    Outcome(Outcome&& temp)
        OGLPLUS_NOEXCEPT(true)
        : BaseOutcome(static_cast<BaseOutcome&&>(temp)), _value(std::move(temp._value))
    {
    }
#endif
...
public:
    ...
    Outcome(Outcome&& temp)
        OGLPLUS_NOEXCEPT(true)
        : BaseOutcome(static_cast<BaseOutcome&&>(temp)), _ptr(std::move(temp._ptr))
    {
    }
#endif
    ...
    Outcome(DeferredHandler handler)
        : BaseOutcome(std::move(handler)), _ptr(nullptr)
    {
    }

    Outcome(DeferredHandler handler, T& ref)
        OGLPLUS_NOEXCEPT(true)
        : BaseOutcome(std::move(handler)), _ptr(&ref)
    {
    }
...
public:
    ...
    bool operator!(void) const OGLPLUS_NOEXCEPT(true)
    {
...
template <typename T>
static inline PositiveOutcome<T> Succeeded(Outcome<T>&& outcome)
    OGLPLUS_NOEXCEPT(true)
{
...
template <typename T>
static inline NegativeOutcome<T> Failed(Outcome<T>&& outcome)
    OGLPLUS_NOEXCEPT(true)
{
    return std::move(outcome);
}
