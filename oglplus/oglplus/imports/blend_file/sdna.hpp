...
private:
    ...
    public:
        ...
        _flat_struct_info(std::size_t field_count)
            : _field_names(field_count), _field_structs(field_count), _field_indices(field_count), _field_offsets(field_count)
        {
        }
        ...
        {
            assert(
                _field_names.size() == _field_structs.size());
            assert(
                _field_names.size() == _field_indices.size());
            assert(
                _field_names.size() == _field_offsets.size());
    ...
    struct _struct_info {
        ...
        {
            assert(
                _field_type_indices.size() == _field_name_indices.size());
            assert(
                _field_type_indices.size() == _field_elem_counts.size());
            assert(
                _field_type_indices.size() == _field_ptr_flags.size());
            assert(
                _field_type_indices.size() == _field_ptr2_flags.size());
            assert(
                _field_type_indices.size() == _field_array_flags.size());
