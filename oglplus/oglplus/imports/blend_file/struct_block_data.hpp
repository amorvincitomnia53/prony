...
protected:
    BlendFileFlatStructTypedFieldDataImpl(
        ...
        std::size_t offset) : _flat_field(std::move(flat_field)), _block_data_ref(&block_data_ref), _offset(offset)
    {
    }

    BlendFileFlatStructTypedFieldDataImpl(
        BlendFileFlatStructTypedFieldDataImpl&& tmp) : _flat_field(std::move(tmp._flat_field)), _block_data_ref(tmp._block_data_ref), _offset(tmp._offset)
    {
    }
...
private:
    ...
    BlendFileFlatStructTypedFieldData(
        BlendFileFlattenedStructField&& flat_field,
        const BlendFileBlockData& block_data_ref,
        std::size_t offset) : _base(std::move(flat_field), block_data_ref, offset)
    {
    }
...
private:
    ...
    BlendFileFlatStructBlockData(
        ...
        std::size_t offset) : _flat_struct(std::move(flat_struct)), _block(std::move(block)), _data(std::move(data)), _offset(offset)
    {
    }
...
public:
    BlendFileFlatStructBlockData(BlendFileFlatStructBlockData&& tmp)
        : _flat_struct(std::move(tmp._flat_struct)), _block(std::move(tmp._block)), _data(std::move(tmp._data)), _offset(tmp._offset)
    {
    }
