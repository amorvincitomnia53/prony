...
public:
    ...
    ShapeWrapperBase(
        ...
        Iterator names_end,
        const ShapeBuilder& builder,
        Selector selector) : _face_winding(builder.FaceWinding()), _shape_instr(builder.Instructions(selector)), _index_info(builder), _vbos(std::size_t(std::distance(names_begin, names_end) + 1)), _npvs(std::size_t(std::distance(names_begin, names_end) + 1), 0), _names(std::size_t(std::distance(names_begin, names_end)))
    {
        this->_init(
            builder,
    ...
    ShapeWrapperBase(ShapeWrapperBase&& temp)
        : _face_winding(temp._face_winding), _shape_instr(std::move(temp._shape_instr)), _index_info(temp._index_info), _gl(std::move(temp._gl)), _vao(std::move(temp._vao)), _vbos(std::move(temp._vbos)), _npvs(std::move(temp._npvs)), _names(std::move(temp._names))
    {
    }
...
public:
    ...
    ShapeWrapperTpl(
        ...
        const ShapeBuilder& builder) : ShapeWrapperBase(names.begin(), names.end(), builder, _sel())
    {
    }
    ...
    ShapeWrapperTpl(
        const StdRange& names,
        const ShapeBuilder& builder,
        const ProgramOps& prog) : ShapeWrapperBase(names.begin(), names.end(), builder, _sel())
    {
        UseInProgram(prog);
    }
    ...
    ShapeWrapperTpl(
        const GLchar** names,
        unsigned name_count,
        const ShapeBuilder& builder) : ShapeWrapperBase(names, names + name_count, builder, _sel())
    {
    }
    ...
    ShapeWrapperTpl(
        ...
        unsigned name_count,
        const ShapeBuilder& builder,
        const ProgramOps& prog) : ShapeWrapperBase(names, names + name_count, builder, _sel())
    {
        UseInProgram(prog);
    }
    ...
    ShapeWrapperTpl(
        const GLchar* name,
        const ShapeBuilder& builder) : ShapeWrapperBase(&name, (&name) + 1, builder, _sel())
    {
    }
    ...
    ShapeWrapperTpl(
        const GLchar* name,
        const ShapeBuilder& builder,
        const ProgramOps& prog) : ShapeWrapperBase(&name, (&name) + 1, builder, _sel())
    {
        UseInProgram(prog);
    }
};
