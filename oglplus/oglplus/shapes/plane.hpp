...
public:
    ...
    Plane(void)
        : _point(0.0f, 0.0f, 0.0f), _u(1.0f, 0.0f, 0.0f), _v(0.0f, 0.0f, -1.0f), _udiv(2), _vdiv(2)
    {
    }

    /// Creates a default plane with udiv and vdiv divisions
    Plane(unsigned udiv, unsigned vdiv)
        : _point(0.0f, 0.0f, 0.0f), _u(1.0f, 0.0f, 0.0f), _v(0.0f, 0.0f, -1.0f), _udiv(udiv), _vdiv(vdiv)
    {
        assert(udiv > 1);
        assert(vdiv > 1);
    }
    ...
    Plane(const Vec3f u, const Vec3f v)
        : _point(0.0f, 0.0f, 0.0f), _u(u), _v(v), _udiv(2), _vdiv(2)
    {
    ...
    Plane(
        ...
        const Vec3f v,
        unsigned udiv,
        unsigned vdiv) : _point(p), _u(u), _v(v), _udiv(udiv), _vdiv(vdiv)
    {
        assert(Length(_u) > 0.0f);
        assert(Length(_v) > 0.0f);
        assert(_udiv != 0);
