...
public:
    ProgramInterfaceContext(
        GLuint prog_name,
        GLuint size,
        GLenum stage_or_intf = GL_NONE) : _prog_name(prog_name), _size(size), _stage_or_intf(stage_or_intf)
    {
    }

    ProgramInterfaceContext(ProgramInterfaceContext&& tmp)
        : _prog_name(tmp._prog_name), _size(tmp._size), _stage_or_intf(tmp._stage_or_intf), _buffer(std::move(tmp._buffer))
    {
    }
...
private:
    ...
    ActiveVariableInfo(
        ...
        GLint size,
        GLenum type,
        const String& var_name) : _index(index), _size(size), _type(type), _var_name(var_name)
    {
    }
