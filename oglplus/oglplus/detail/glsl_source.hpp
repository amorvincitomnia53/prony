...
public:
    StrCRefGLSLSrcWrap(const StrCRef& source)
        : _ptr(source.begin()), _size(GLint(source.size()))
    {
        assert(_ptr != nullptr);
    }
...
public:
    StrGLSLSrcWrap(const String& source)
        : _storage(source), _ptr(_storage.c_str()), _size(GLint(_storage.size()))
    {
        assert(_ptr != nullptr);
    }

    StrGLSLSrcWrap(String&& source)
        : _storage(std::move(source)), _ptr(_storage.c_str()), _size(GLint(_storage.size()))
    {
        assert(_ptr != nullptr);
    }
