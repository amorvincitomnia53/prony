...
public:
    AlignedPODArray(void)
        : _count(0), _sizeof(0), _data(nullptr), _delete(nullptr), _dup(nullptr)
    {
    }

    template <typename T>
    AlignedPODArray(const T* data, std::size_t count)
        : _count(count), _sizeof(sizeof(T)), _data(_do_dup<T>(static_cast<const void*>(data), count)), _delete(&_do_delete<T>), _dup(&_do_dup<T>)
    {
    }

    AlignedPODArray(AlignedPODArray&& tmp)
        : _count(tmp._count), _sizeof(tmp._sizeof), _data(tmp._release_data()), _delete(tmp._delete), _dup(tmp._dup)
    {
    }

    AlignedPODArray(const AlignedPODArray& that)
        : _count(that._count), _sizeof(that._sizeof), _data(that._data_copy()), _delete(that._delete), _dup(that._dup)
    {
    }
