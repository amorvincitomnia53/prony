...
public:
    ...
    ContextElementRange(Context&& context, unsigned current, unsigned count)
        : _context(std::move(context)), _current(current), _count(count)
    {
        assert(current <= count);
    }

    ContextElementRange(Context&& context, unsigned count)
        : _context(std::move(context)), _current(0), _count(count)
    {
    }

    ContextElementRange(ContextElementRange&& tmp)
        : _context(std::move(tmp._context)), _current(tmp._current), _count(tmp._count)
    {
    }
...
public:
    ...
    ConvIterRange(Converter conv, Iterator begin, Iterator end)
        : IterRange<Iterator>(begin, end), _conv(conv)
    {
    }
