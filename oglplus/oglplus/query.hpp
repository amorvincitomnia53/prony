...
public:
    ...
    void Begin(Target target)
    {
        ...
        (GLenum(target), _obj_name());
        OGLPLUS_VERIFY(
            BeginQuery,
            ObjectError,
            Object(*this).EnumParam(target));
    }
...
public:
    ...
    QueryActivator(
        QueryName query,
        QueryTarget target) : _query(query), _target(target), _alive(false)
    {
    ...
    QueryActivator(QueryActivator&& temp)
        : _query(std::move(temp._query)), _target(temp._target), _alive(temp._alive)
    {
        temp._alive = false;
    }
...
public:
    ...
    ConditionalRender(
        QueryName query,
        ConditionalRenderMode mode) : _alive(false)
    {
...
public:
    QueryExecution(
        QueryName query,
        QueryTarget target,
        ResultType& result) : QueryActivator(query, target), _result(result)
    {
    }

    QueryExecution(QueryExecution&& temp)
        : QueryActivator(static_cast<QueryActivator&&>(temp)), _result(temp._result)
    {
    }
