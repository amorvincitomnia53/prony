...
private:
    ...
    {
        return _v1 * Sin((T(1) - t) * _omega) * _inv_sin_omega + _v2 * Sin(t * _omega) * _inv_sin_omega;
    }
...
public:
    ...
    BaseSLERP(
        const Value& v1,
        const Value& v2,
        T eps) : _v1(v1), _v2(v2), _omega(Angle<T>::ArcCos(Dot(_v1, _v2))), _inv_sin_omega(Sin(_omega)), _func(nullptr)
    {
