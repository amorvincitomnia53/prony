...
public:
    ...
    CameraMatrix(
        ...
        const Vector<T, 3>& target) : Base(oglplus::Nothing())
    {
        assert(eye != target);
    ...
    CameraMatrix(
        ...
        const Vector<T, 3>& eye,
        const Vector<T, 3>& target,
        const Vector<T, 3>& up) : Base(oglplus::Nothing())
    {
        assert(eye != target);
        Vector<T, 3> z = Normalized(eye - target);
        T dupz = Dot(up, z);
