...
public:
        ...
        TextureName texture)
    {
        ...
        OGLPLUS_VERIFY(
            BindTexture,
            ObjectError,
            Object(texture).BindTarget(target));
    }
...
class ObjCommonOps<tag::Texture>
    : public TextureName, public ObjBindingOps<tag::Texture>
{
protected:
...
public:
    ...
    static void CompareMode(Target target, TextureCompareMode mode)
    {
        ...
        OGLPLUS_CHECK(
            TexParameteri,
            ObjectError,
            ObjectBinding(target).EnumParam(mode));
    }
    ...
    static void CompareFunc(Target target, CompareFunction func)
    {
        ...
        OGLPLUS_CHECK(
            TexParameteri,
            ObjectError,
            ObjectBinding(target).EnumParam(func));
    }
    ...
    static void Filter(Target target, TextureFilter filter)
    {
        ...
        OGLPLUS_CHECK(
            TexParameteri,
            ObjectError,
            ObjectBinding(target).EnumParam(filter));
        OGLPLUS_GLFUNC(TexParameteri)
        (
            GLenum(target),
...
#if OGLPLUS_DOCUMENTATION_ONLY || GL_VERSION_4_5 || GL_ARB_texture_barrier || GL_NV_texture_barrier
